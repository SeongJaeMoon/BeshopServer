...
import static app.cap.beshopserver.pProtocol.decrypt;
import static app.cap.beshopserver.pProtocol.hmacSha1;
import static app.cap.beshopserver.pProtocol.xor;
...
//            System.exit(0);
    ...
    public boolean verifyZKP(BigInteger p, BigInteger q, BigInteger g, BigInteger gx,
        ...
        if (gx.compareTo(BigInteger.ZERO) == 1 && // g^x > 0
    			...
    			/* 아래에서,  2^exp를 필요로하는 g^r*g^x^h를 계산하는 간단한 방법을 사용
    			*사용된 동시 계산 기법은 1 Exp 만 필요
    			*/
    ...
    public BigInteger getSHA1(BigInteger g, BigInteger gr, BigInteger gx, String signerID) {
        ...
        try {
    		...
    		/* Note: H (...)의 항목에 명확한 경계가 있는지 확인해야합니다.
    		 * 상대방이 g, gr, gx의 크기를 알고 있으면 간단합니다.
    		 * signerID 따라서 경계는 모호하지 않습니다
    		 * 각 항목의 바이트 길이를 더하지 않고 여기서는 그 값을 무시
    		 */
    ...
    public BigInteger getSHA1(BigInteger K) {
        ...
        return new BigInteger(1, sha.digest()); // 양수 int의 경우 1
    }
